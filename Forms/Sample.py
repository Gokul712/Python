...
xl_File = sys.argv[1]
coveragefile = os.getcwd()+"/sample/Coverages.csv"
# print(xl_File)
OU = sys.argv[2]
...
#    set(productcode)
   ...
   elif isinstance(co['State'], str) and str(co['State']) != ' ':
...
#    no_state = str(co['NoState'])
   ...
   for s,cov in coverage_name.iterrows():      
      ...
      # print(str(cov['LOB']).strip(), end='------------')
      # print(prod)
      # print(str(cov['CoverageName']).lower().replace(" ", "").replace("-","").replace("–",""), end='------------------')
      # print(str(co['Description']).lower().replace(" ", "").replace("-","").replace("–",""))
      # if(str(cov['CoverageName']) == 'Exclusion - Products Prior to Specified Date'):
      #    print(str(cov['LOB']).strip(), end='------------')
      #    print(prod)
      #    print(str(cov['CoverageName']).lower().replace(" ", "").replace("-","").replace("–",""), end='------------------')
      #    print(str(co['Description']).lower().replace(" ", "").replace("-","").replace("–",""))
      #    print(str(cov['LOB']).strip() == prod)
      #    print(str(cov['CoverageName']).lower().replace(" ", "").replace("-","").replace("–","") == str(co['Description']).lower().replace(" ", "").replace("-","").replace("–",""))
...
#    print(Code.text)
   ...
   #FormPatternOU_Ext
   FormPatternOU_Ext = ET.SubElement(FormPattern, 'FormPatternOU_Ext')
   fp_OU = ET.SubElement(FormPatternOU_Ext,'FormPatternOU_Ext',{'public-id':str(uuid.uuid4())})
   Avl = ET.SubElement(fp_OU,'Availability_Ext')
   Avl.text = 'Unavailable'
   ouForm = ET.SubElement(fp_OU,'FormPattern_Ext',{'public-id':Form_publicID})
   operatingUnit = ET.SubElement(fp_OU,'OperatingUnit_Ext')
   # if prod in ['EPLLine','DOLLine','EOLine','FIDLine',None]:
   operatingUnit.text = OU
   ...
   if no_State is not None:
    ...
    #    if isinstance(co['Company'],str) and ',' not in co['Company']:
    #      uwcomp.text =  underwriting_comp[co['Company']] 
   not_line = ['CPP7CommercialPackage','ESPackage']
